...
def dropout1(tensor_in=None, layer=0, params=None, mtrain=None):
    ...
    with tf.variable_scope("dropout1_"+str(layer)) as scope:
        tensor_out = tf.cond(mtrain, lambda: tf.nn.dropout(tensor_in, keep_p), lambda: tf.identity(tensor_in))
        print_activations(tensor_out)    
    return tensor_out
...
def global_pool1(tensor_in=None, layer=0, params=None, mtrain=None):
    ...
    with tf.variable_scope("global_pool1_"+str(layer)) as scope:
