...
class ProposalsTargetLayer(object):
    ...
    def __init__(self, img_shp=None):
        ...
        self.roi_msk_siz = [28, 28]
        self.roi_msk_min = 0.5
        
    #如果是Faster RCNN，就用roi_imxs把每幅图片的rois合并起来!!!    
    def sample_rois_img(self, elems=None):
        
        rois, roi_num, gbxs, gmks, gbx_num = elems
        rois = rois[0:roi_num]
        gbxs = gbxs[0:gbx_num]
        gmks = gmks[0:gbx_num]
        ...
        fgd_msks_pre = tf.cast(fgd_msks_pre>=self.roi_msk_min, dtype=tf.float32) #(M0, 21, 21)
    ...
    def sample_rois(self, rois, roi_nums, gbxs, gmks, gbx_nums):
        
        elems = [rois, roi_nums, gbxs, gmks, gbx_nums]
        self.fbg_rois, self.fbg_prbs_pre, self.fgd_prds_pre, self.fgd_msks_pre, self.fgd_nums, self.fbg_nums = \
            tf.map_fn(self.sample_rois_img, elems, dtype=(tf.float32, tf.int32, tf.float32, tf.float32, tf.int32, tf.int32),
    ...
    def generate_roi_loss_img(self, elems=None):
        ...
        fgd_msks_pst = tf.gather_nd(fbg_msks_pst, fgd_idxs)       #(M0, 21, 21) #(M, C, 21, 21) #(M0, 21, 21, 2) #(M, C, 21, 21, 2)
        #roi_msks_los = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=fgd_msks_pre, logits=fgd_msks_pst) #和fgd_idxs对应
        roi_msks_los = tf.nn.sigmoid_cross_entropy_with_logits(labels=fgd_msks_pre, logits=fgd_msks_pst) #(M0, 21, 21) dtype相同!!!
        ...
        fgd_prds_pst = fbg_prds_pst[0:fgd_num]                    #(M0, C, 4)
        fgd_prds_pst = tf.gather_nd(fgd_prds_pst, fgd_idxs)       #(M0, 4)
    ...
    def generate_roi_loss(self, roi_prbs_pst=None, roi_prds_pst=None, roi_msks_pst=None):
        ...
        roi_prbs_los = tf.cond(fbg_num>0, lambda: roi_prbs_los/fbg_num, lambda: tf.constant(0.0))
        roi_prds_los = tf.cond(fbg_num>0, lambda: roi_prds_los/fbg_num, lambda: tf.constant(0.0))
        roi_msks_los = tf.cond(fbg_num>0, lambda: roi_msks_los/fbg_num, lambda: tf.constant(0.0)) * 10.0
