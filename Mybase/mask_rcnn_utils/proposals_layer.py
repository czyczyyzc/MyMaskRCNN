...
class ProposalsLayer(object):
    ...
    def __init__(self, mod_tra=True, rpns=None, img_shp=None):
        ...
        self.roi_siz_min = 5.0
        self.roi_prb_min = None                            
        self.roi_nms_pre = 12000 if self.mod_tra else 6000 
        self.roi_nms_pst =  2000 if self.mod_tra else 600 
        self.roi_nms_max = 0.8
    ...
    def generate_rois_img(self, elems=None):
        ...
        roi_idxs = tf.range(tf.shape(self.rpns)[0])
        
        #剔除得分较低的roi
        if self.roi_prb_min is not None:
            idxs     = tf.where(roi_prbs>=self.roi_prb_min)
            roi_idxs = tf.gather_nd(roi_idxs, idxs)
            roi_prbs = tf.gather_nd(roi_prbs, idxs)
        #进一步剔除过多的roi
        if self.roi_nms_pre is not None:
            roi_nms_pre    = tf.minimum(self.roi_nms_pre, tf.shape(roi_idxs)[0])
            roi_prbs, idxs = tf.nn.top_k(roi_prbs, k=roi_nms_pre, sorted=True)
            roi_idxs       = tf.gather(roi_idxs, idxs)
        #根据roi_idxs进行剩余的gather操作
        rpns         = tf.gather(self.rpns,    roi_idxs)
        rpn_prds_pst = tf.gather(rpn_prds_pst, roi_idxs)
        #还原出roi以进行后续的滤除
    ...
    def generate_rois(self, rpn_prbs_pst=None, rpn_prds_pst=None):
        ...
        #roi_nums = tf.Print(roi_nums, [roi_nums], message=None, first_n=None, summarize=None) 
